...
def load_new_data(root_dir, img_name='mri', mask_name='masks', size=(256, 256, 64), return_paths=False):

    paths = [img_path for img_path in sorted(
        os.listdir(root_dir)) if img_path[-4:] == '.mat']

    # read all .mat files
    X = [resize(readmat(root_dir + img_path, img_name), size)
            for i, img_path in tqdm(enumerate(paths), total=len(paths))]
    X = np.expand_dims(np.stack(X), -1)

    Y = [resize(readmat(root_dir + img_path, mask_name), size)
            for i, img_path in tqdm(enumerate(paths), total=len(paths))]
    Y = np.expand_dims(np.stack(Y), -1)

    Yc = [Y == c for c in [0, 1, 2, 4, 8, 16]]
    Yc = np.concatenate(Yc, axis=-1)
    if return_paths:
        return X, Yc, paths
    else:
        return X, Yc
